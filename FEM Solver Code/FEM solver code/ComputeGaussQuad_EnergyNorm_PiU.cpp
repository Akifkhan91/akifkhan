...
{
	double xi[2];
	double GradN[2], Grad_Uexact[2], Grad_PiU[2] = {0.0, 0.0};
	...
	for(int i = 0; i < SIZE_XIPTS; i++)
	{
		...
		ErrorEnergyNorm = (Grad_Uexact[0] - Grad_PiU[0]) * (Grad_Uexact[0] - Grad_PiU[0]) +
						  (Grad_Uexact[1] - Grad_PiU[1]) * (Grad_Uexact[1] - Grad_PiU[1]);
